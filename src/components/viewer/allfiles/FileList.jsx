...
const ShareModal = ({
  ...
  const fileUrl = `${REACT_APP_BACKEND_URL}/documents/${file.filename}`;
...
const FileCard = ({
  ...
  const authenticatedHandleDownload = () => {
    const fileUrl = `${REACT_APP_BACKEND_URL}/documents/${file.filename}`;
  ...
  const authenticatedHandlePdfView = () => {
    const pdfUrl = `${REACT_APP_BACKEND_URL}/documents/${file.filename}`;
  ...
  return (
    ...
    >
      <div className={`${isListView ? "w-full sm:w-1/4" : "w-full"} p-2`}>
        <Thumbnail
          file={`${REACT_APP_BACKEND_URL}/documents/${file.filename}`}
      ...
      <div
        className={`${
          isListView
            ? "w-full sm:w-3/4 flex flex-col justify-between"
            : "w-full"
        } p-4`}
      >
        <div>
          ...
          <div className={`${isListView ? "flex flex-wrap" : ""}`}>
            ...
            <DetailRow
              icon={Calendar}
              label="Published"
              value={formatDate(file.publicationDate)}
            />
          ...
          <ExpandableDescription
            ...
            maxLength={isListView ? 100 : 50}
          />
        </div>
        <div
          className={`flex flex-wrap justify-between items-center ${
            isListView ? "mt-4" : "mt-2"
          }`}
        >
          <button
            onClick={authenticatedHandleDownload}
          ...
          <div className="flex space-x-2">
            <button
              onClick={authenticatedHandlePdfView}
            ...
            <button
              onClick={() => onOpenCommentsModal(file._id)}
              className="text-gray-600 hover:text-amber-600"
            >
...
// Main FileList Component (continued)
const FileList = ({
  currentFiles,
  handleDownload,
  handlePdfView,
  ...
  const handleOpenCommentsModal = (documentId) => {
